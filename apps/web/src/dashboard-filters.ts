...
export type DashboardFilters = { year?: string; region?: string; institution?: string; specialty?: string };
...
export function filterRows(rows: DashboardRow[], filters: DashboardFilters): DashboardRow[] {
  return rows.filter(row =>
    ...
    (!filters.specialty || (row as DashboardRow & { specialty?: string }).specialty === filters.specialty),
  );
}

export function filterOptions(rows: DashboardRow[]) {
  const unique = (values: string[]) => [...new Set(values)].sort((a, b) => a.localeCompare(b, 'uk'));
  return {
