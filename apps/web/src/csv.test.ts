...
describe('parseStatisticsCsv', () => {
  it('parses a valid statistics dataset', () => expect(parseStatisticsCsv(valid)).toEqual([{
    institution: 'Demo',
    region: 'Kyiv',
  ...
  it('rejects rows with a missing field', () => expect(() => parseStatisticsCsv(valid.replace('Kyiv', ''))).toThrow('Порожнє поле'));
  it('rejects unclosed quotes', () => expect(() => parseStatisticsCsv(valid.replace('Demo', '"Demo'))).toThrow('Незакриті лапки'));
});
